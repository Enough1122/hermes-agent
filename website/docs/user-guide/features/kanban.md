...
| `POST` | `/tasks/:id/reassign` | Reassign with optional reclaim-first (recovery popover) |
...
The dashboard's session hash is a short SHA-256 prefix of the server's ephemeral session token — stable per dashboard process, never the raw token. These strings are **audit metadata, not authenticated principals**: they correlate actions to a first-party surface/process but do not grant authority or prove a human identity. Events written by callers predating this field (or by third-party tools calling `kanban_db` directly) simply carry no `operator` key; nothing else about the payload changed, so old consumers and old rows are unaffected. Read the trail with `hermes kanban show <id>` (events section) or `hermes kanban tail`.

...
# ---------------------------------------------------------------------------
# Operator attribution (#82689)
# ---------------------------------------------------------------------------


def test_patch_assign_stamps_operator(client):
...
def test_dashboard_operator_upgrades_fallback_after_server_token_exists(monkeypatch):
    """Import order must not permanently cache the host:pid fallback."""
    import hermes_cli.web_server as web_server
    from plugins.kanban.dashboard import plugin_api

    plugin_api._DASHBOARD_OPERATOR = None
    monkeypatch.setattr(web_server, "_SESSION_TOKEN", None)
    fallback = plugin_api._dashboard_operator()
    assert fallback.startswith("dashboard:")
    assert plugin_api._DASHBOARD_OPERATOR is None

    monkeypatch.setattr(web_server, "_SESSION_TOKEN", "later-session-token")
    token_identity = plugin_api._dashboard_operator()
    assert token_identity.startswith("dashboard:")
    assert token_identity != fallback
    assert plugin_api._DASHBOARD_OPERATOR == token_identity
